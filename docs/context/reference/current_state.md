...
- T20 modifies the label layer; the later retrain covers segments and extends reporting.
...
**Diagnosis — tautology. CONFIRMED, not hypothesised** (user's prior testing —
already established, no task needed):
...
string predict them (T09)? — not on incremental AUC. This finding is settled; do not
spend a task re-establishing it.
...
- `CASA inflow/outflow` + `bureau total salary` — real ability signal. But importance
  ...
  **T24** (CASA-holder vs card-only performance split) decides what Axis A1 is worth.
...
|---|---|---|
| **Restructure = account succession** (R13): the old CC/loan is closed and a new one opened. If closure posts a settlement-like credit, a restructure reads as **recovery** — inverted, since the debt moved rather than being repaid. The old→new link is not yet identifiable (O13) | T17 / T18 | Recovery and futility labels inflated on every restructured account; `payments()` must exclude succession postings (`feature_specs.md §1`); restructured customers are an A4 blind spot until T18/T19 lands |
| Card block codes fire after observation date, or are DPD-triggered operational actions | T21 | Leakage or self-fulfilling feature — drop or lag it |
| Bureau salary freshness inconsistent across rows (AECB dry at delinquency) | T21 | Feature means different things per row; model exploits the inconsistency |
