...
| Auto | Loan system | **Yes** | The one sub-split that is *easy* within loans. Recovery = repossess/sell asset; persona axes mostly n/a (auto track). Ijarah vs lien unknown (O3) |
...
| S18 | **A2 is derived, not learned** (`feature_specs §4.0`). States resolve from contact-outcome status × presence evidence. Field visits are the **adjudicator** of one ambiguous cell and the validator of `gone` precision — never the training set for the whole book, because visits are dispatched *because* contact already failed. Supervised refinement is scoped by coverage, not a prerequisite. |
| S19 | **UC1 will supply ability and willingness from call transcripts.** A1/A3 stay defined now so UC1 lands into an existing schema. Consequences: A3 is *deferred*, not permanently blocked on DCORE PTP quality; **A2's `reachable` population is UC1's addressable book** (transcripts exist only where someone answered); and UC1 can never cover the unreached, so A4 remains the only axis with coverage there — complementary, not sequential. The field-visit selection lesson applies to transcript-derived labels too. |
| S20 | **A class label never ships without its provenance.** Wherever an axis state reaches an agent-facing surface, the confidence value and `observed\|inferred` flag travel with it. The A4 names are legible enough to be over-read as diagnosed causes (`personas.md §5`). |
...
| O15 | **Does the "answered but no meaningful conversation" outcome distinguish the customer deflecting from a stranger on a reassigned number?** If it is one undifferentiated code, that row of the §4.0 matrix collapses and `avoiding` / `skip` both inherit the ambiguity | A2 state separation, T12 | Collections / IT |
| O16 | **How are field visits actually dispatched** — by failed contact, by balance, or by a routing rule? And how much visit history exists? | The §4.0 selection argument; whether §4.3's refinement is worth building | Collections |
| O17 | **Do loan month-end snapshots carry a days-overdue column?** If so, which DPD convention (oldest unpaid instalment vs most recent miss), how are partial payments applied, DPD-at-snapshot or max-in-month, and what is the days→bucket boundary? Also: snapshot history depth | Loan spell/T0 derivation (`feature_specs §2`), T25 | User — **answerable in UAT from workplace schema docs** |
| O18 | **Per-source retention depth** for the CC payment, balance and utilisation tables, separately from transaction history | A4 window per feature family (`feature_specs §3.0b`), T02 | User |
